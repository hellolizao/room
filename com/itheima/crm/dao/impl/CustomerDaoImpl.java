...
public class CustomerDaoImpl extends HibernateDaoSupport implements CustomerDao {
	...
	public Integer findCount(DetachedCriteria detachedCriteria) {
		...
		if(list.size()>0) {
			return list.get(0).intValue();//list中只有关于客户的记录数，没有别的记录数（将Long转为int类型
		}
		return null;
	}
	...
	@Override
	public Customer findById(Long cust_id) {//进行删除前的查询
	...
	@Override
	public void delete(Customer customer) {//将查询得到的客户删除
		this.getHibernateTemplate().delete(customer);
		
	}

	@Override
	public void update(Customer customer) {//客户进行修改后的重新的保存
